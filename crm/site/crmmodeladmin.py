...
class CrmModelAdmin(BaseModelAdmin):
        ...
        if request.user.is_superuser or request.user.is_superoperator:
            ...
            if db_field.name in ("owner", "co_owner"):
                kwargs["queryset"] = (
                    ...
                    .filter(
                        groups__name__in=("managers", "operators", "superoperators")
                    )
                    .distinct()
                )
            elif db_field.name in _fields:
    ...
    def get_list_filter(self, request):
        ...
        if self.model in (Deal, Request, Company, Contact, Lead):
            ...
            if hasattr(self.model, "city"):
                if (
                    "country__id__exact" in request.GET
                    or "city__id__exact" in request.GET
                    or not country_filter_needed
                ):
    ...
    def get_queryset(self, request):
        ...
        if request.user.department_id:
            ...
            return qs.filter(
                department__in=request.user.groups.filter(department__isnull=False)
            )
        return qs
    ...
    def create_email(self, obj):
        if obj.email and obj.id:
            ...
            url = (
                reverse("create_email", args=(obj.id,))
    ...
    def make_massmail(self, request):
        ...
        if request.method == "POST":
            ...
            if not have_massmail_accounts(request):
                return HttpResponseRedirect(reverse("site:app_list", args=("crm",)))
            min_date = None
            if queryset:
                min_date = queryset.aggregate(min_date=Min("creation_date"))[
                    "min_date"
                ].date()
    ...
    @staticmethod
    @admin.display(description=mark_safe(email_icon.format(email_title)))
        ...
        if obj.massmail:
            if not obj.disqualified:
                ...
                is_mcs = MailingOut.objects.filter(
                    content_type=content_type,
                    successful_ids__regex=rf"(^|,){id_str}(,|$)",
                )
                if not is_mcs:
                    return mark_safe(did_not_receive_icon.format(did_not_receive_title))
            return mark_safe(subscribed_icon.format(subscribed_title))
        return mark_safe(unsubscribed_icon.format(unsubscribed_title))
    ...
    def the_city(self, obj):
        ...
        if not value:
            url = self.get_url_for_callable("city__id__exact", obj.city.id)
    ...
    def the_country(self, obj):
        ...
        value = getattr(_thread_local, url_name, None)
        if not value:
            url = self.get_url_for_callable("country__id__exact", obj.country.id)
    ...
    @admin.display(
        description=mark_safe(
            f"{_('Mass mailing')} " f"{unsubscribed_icon.format(unsubscribed_title)}"
        )
    )
    ...
    def get_country_filter_needed(self, request: WSGIRequest) -> bool:
        ...
        if country_filter_needed is None:
            ...
            num_countries = (
                qs.exclude(country__isnull=True).values("country").distinct().count()
            )
            if num_countries <= 1:
    ...
    def get_latest_emails(field: str, object_id: int) -> QuerySet:
        ...
        emails = (
            CrmEmail.objects.filter(**kwargs)
            .order_by("-creation_date")[:4]
            .annotate(
                is_attachment=Exists(
                    TheFile.objects.filter(
                        content_type__pk=crmemail_type.id, object_id=OuterRef("pk")
                    )
                )
            )
        )
    ...
    def set_queryset(
        self, request: WSGIRequest, kwargs, model, order_by_field: str = ""
    ) -> None:
        try:
            ...
            kwargs["queryset"] = model.objects.filter(department_id=department_id)
            if order_by_field:
                kwargs["queryset"] = kwargs["queryset"].order_by(order_by_field)
        except KeyError:
            pass
