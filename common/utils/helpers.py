...
def add_chat_context(request, extra_context, object_id, content_type):
    chat = ChatMessage.objects.filter(object_id=object_id, content_type=content_type)
    extra_context["is_chat"] = chat.exists()
    if extra_context["is_chat"]:
        extra_context["is_unread_chat"] = chat.filter(recipients=request.user).exists()
...
def get_active_users() -> QuerySet:
    return USER_MODEL.objects.exclude(Q(is_active=False))
...
def get_manager_departments():
    ...
    return (
        ...
        .objects.filter(department__isnull=False, user__groups__name="managers")
        .distinct()
    )


def get_department_id(user):
    department = user.groups.filter(department__isnull=False).first()
    return department.id if department else None


def get_formatted_short_date():
    return date_format(get_today(), format="SHORT_DATE_FORMAT", use_l10n=True)
...
def send_crm_email(subject: str = "", body: str = "", to: list = None) -> None:
